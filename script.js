...
function init() {
    ...
    const FootprintSeriesDefinition = createFootprintSeriesDefinition();
    footprintSeries = chart.addCustomSeries(FootprintSeriesDefinition, { priceScaleId: 'right' });
...
// Определение кастомной серии (объект, а не класс)
function createFootprintSeriesDefinition() {
    ...
    return {
        ...
        update: function(data) { this._data = data; },
...
function updateCandleWithTrade(candle, price, qty, isAsk) {
    ...
    if (!candle.levels.has(levelPrice)) candle.levels.set(levelPrice, { bidVol: 0, askVol: 0 });
    const level = candle.levels.get(levelPrice);
    if (isAsk) { level.askVol += qty; candle.askVol += qty; }
    else { level.bidVol += qty; candle.bidVol += qty; }
}

function finalizeCandle(candle) {
    const levels = Array.from(candle.levels.entries()).map(([price, vol]) => ({
    ...
    const finalized = {
        ...
        bidVol: candle.bidVol, askVol: candle.askVol, levels
    };
